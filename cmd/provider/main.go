...
import (
	...
	"github.com/ansible/terraform-provider-aap/xpprovider"
	...
	apisCluster "github.com/crossplane-contrib/provider-aap/apis/cluster"
	apisNamespaced "github.com/crossplane-contrib/provider-aap/apis/namespaced"
	"github.com/crossplane-contrib/provider-aap/config"
	"github.com/crossplane-contrib/provider-aap/internal/clients"
	controllerCluster "github.com/crossplane-contrib/provider-aap/internal/controller/cluster"
	controllerNamespaced "github.com/crossplane-contrib/provider-aap/internal/controller/namespaced"
	"github.com/crossplane-contrib/provider-aap/internal/features"
	"github.com/crossplane-contrib/provider-aap/internal/version"
)
...
func main() {
	var (
		app                     = kingpin.New(filepath.Base(os.Args[0]), "Terraform based Crossplane provider for AnsibleAutomationPlatform").DefaultEnvars()
	...
	log := logging.NewLogrLogger(zl.WithName("provider-aap"))
	if *debug {
	...
	mgr, err := ctrl.NewManager(cfg, ctrl.Options{
		...
		LeaderElectionID: "crossplane-leader-election-provider-aap",
		Cache: cache.Options{
	...
	kingpin.FatalIfError(apisCluster.AddToScheme(mgr.GetScheme()), "Cannot add cluster-scoped AnsibleAutomationPlatform APIs to scheme")
	kingpin.FatalIfError(apisNamespaced.AddToScheme(mgr.GetScheme()), "Cannot add namespaced AnsibleAutomationPlatform APIs to scheme")
	...
	fwProvider := xpprovider.GetProvider(ctx)
	kingpin.FatalIfError(err, "Cannot get the Terraform framework and SDK providers")
	clusterProvider := config.GetProvider(fwProvider)
	namespacedProvider := config.GetProviderNamespaced(fwProvider)
	...
	if canSafeStart {
		...
		kingpin.FatalIfError(controllerCluster.SetupGated(mgr, clusterOpts), "Cannot setup cluster-scoped AnsibleAutomationPlatform controllers")
		kingpin.FatalIfError(controllerNamespaced.SetupGated(mgr, namespacedOpts), "Cannot setup namespaced AnsibleAutomationPlatform controllers")
	} else {
		log.Info("Provider has missing RBAC permissions for watching CRDs, controller SafeStart capability will be disabled")
		kingpin.FatalIfError(controllerCluster.Setup(mgr, clusterOpts), "Cannot setup cluster-scoped AnsibleAutomationPlatform controllers")
		kingpin.FatalIfError(controllerNamespaced.Setup(mgr, namespacedOpts), "Cannot setup namespaced AnsibleAutomationPlatform controllers")
	}
