...
func main() {
	...
	fmt.Println("Using repo root:", absRootDir)
	ctx := context.Background()
	fwProvider := xpprovider.GetProvider(ctx)
	provider := config.GetProvider(fwProvider)
	dumpGeneratedResourceList(provider, generatedResourceList)
	dumpSkippedResourcesCSV(provider, skippedResourcesCSV)
	pipeline.Run(provider, config.GetProviderNamespaced(fwProvider), absRootDir)
}
...
func dumpGeneratedResourceList(p *ujconfig.Provider, targetPath *string) {
	...
	fmt.Printf("Generating %d resources\n", len(p.Resources))
