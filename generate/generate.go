...
//go:generate go run github.com/crossplane/upjet/v2/cmd/scraper -n ${TERRAFORM_PROVIDER_SOURCE} -r ../.work/terraform-provider-aap/${TERRAFORM_DOCS_PATH} -o ../config/provider-metadata.yaml --prelude-xpath "//text()[contains(., \"description\")]"
...
//go:generate go run github.com/crossplane/upjet/v2/cmd/resolver -g aap.crossplane.io -a github.com/crossplane-contrib/provider-aap/internal/apis -s -p ../apis/cluster
//go:generate go run github.com/crossplane/upjet/v2/cmd/resolver -g aap.m.crossplane.io -a github.com/crossplane-contrib/provider-aap/internal/apis -s -p ../apis/namespaced
